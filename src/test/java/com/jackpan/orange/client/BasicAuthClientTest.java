...
public class BasicAuthClientTest {
    ...
    @Test
    public void basicAuthPluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.basicAuth().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.basicAuth().plugin().start();
     ...
     */
    @Test
    public void basicAuthSelectorListTest() {
        List<Selector> list = orangeRestClient.basicAuth().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void basicAuthSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.basicAuth().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void basicAuthSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.basicAuth().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.basicAuth().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void basicAuthSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.basicAuth().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.basicAuth().selectors().delete(list.get(0).getId());
