...
public class KeyAuthClientTest {
    ...
    @Test
    public void keyAuthPluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.keyAuth().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.keyAuth().plugin().start();
     ...
     */
    @Test
    public void keyAuthSelectorListTest() {
        List<Selector> list = orangeRestClient.keyAuth().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void keyAuthSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.keyAuth().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void keyAuthSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.keyAuth().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.keyAuth().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void keyAuthSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.keyAuth().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.keyAuth().selectors().delete(list.get(0).getId());
