...
public class WafClientTest {
    ...
    // @Test
    public void wafPluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.waf().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.waf().plugin().start();
     ...
     */
    @Test
    public void wafSelectorListTest() {
        List<Selector> list = orangeRestClient.waf().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void wafSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.waf().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void wafSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.waf().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.waf().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void wafSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.waf().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.waf().selectors().delete(list.get(0).getId());
