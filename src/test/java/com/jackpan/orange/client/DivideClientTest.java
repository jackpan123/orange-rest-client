...
public class DivideClientTest {
    ...
    @Test
    public void dividePluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.divide().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.divide().plugin().start();
     ...
     */
    @Test
    public void divideSelectorListTest() {
        List<Selector> list = orangeRestClient.divide().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void divideSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.divide().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void divideSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.divide().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.divide().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void divideSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.divide().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.divide().selectors().delete(list.get(0).getId());
