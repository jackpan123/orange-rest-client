...
public class PropertyRateLimitingClientTest {
    ...
    @Test
    public void rateLimitingPluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.rateLimiting().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.rateLimiting().plugin().start();
     ...
     */
    @Test
    public void rateLimitingSelectorListTest() {
        List<Selector> list = orangeRestClient.rateLimiting().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void rateLimitingSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.rateLimiting().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void rateLimitingSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.rateLimiting().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.rateLimiting().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void rateLimitingSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.rateLimiting().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.rateLimiting().selectors().delete(list.get(0).getId());
