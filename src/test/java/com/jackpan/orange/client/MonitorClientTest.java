...
public class MonitorClientTest {
    ...
    // @Test
    public void monitorPluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.monitor().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.monitor().plugin().start();
     ...
     */
    @Test
    public void monitorSelectorListTest() {
        List<Selector> list = orangeRestClient.monitor().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void monitorSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.monitor().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void monitorSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.monitor().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.monitor().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void monitorSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.monitor().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.monitor().selectors().delete(list.get(0).getId());
