...
public class HmacClientTest {
    ...
    // @Test
    public void hmacAuthPluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.hmacAuth().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.hmacAuth().plugin().start();
     ...
     */
    @Test
    public void hmacAuthSelectorListTest() {
        List<Selector> list = orangeRestClient.hmacAuth().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void hmacAuthSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.hmacAuth().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void hmacAuthSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.hmacAuth().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.hmacAuth().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void hmacAuthSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.hmacAuth().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.hmacAuth().selectors().delete(list.get(0).getId());
