...
public class SignatureAuthClientTest {
    ...
    @Test
    public void signatureAuthPluginTest() {
        // stop plugin
        AcknowledgedResponse stop = orangeRestClient.signatureAuth().plugin().stop();
        if (stop.isAcknowledged()) {
            System.out.println(stop.getMsg());
        }

        AcknowledgedResponse start = orangeRestClient.signatureAuth().plugin().start();
     ...
     */
    @Test
    public void signatureAuthSelectorListTest() {
        List<Selector> list = orangeRestClient.signatureAuth().selectors().list();
        System.out.println();
     ...
     */
    @Test
    public void signatureAuthSelectorCreateTest() {
        // create condition
        ...
        AcknowledgedResponse response = orangeRestClient.signatureAuth().selectors().create(test3);
        System.out.println(response.getMsg());

    }

    @Test
    public void signatureAuthSelectorUpdateTest() {
        List<Selector> list = orangeRestClient.signatureAuth().selectors().list();
        if (list.size() > 0) {
            ...
            AcknowledgedResponse update = orangeRestClient.signatureAuth().selectors().update(selector);
            System.out.println();
        }
    }
     ...
     */
    @Test
    public void signatureAuthSelectorDeleteTest() {
        List<Selector> list = orangeRestClient.signatureAuth().selectors().list();
        if (list.size() > 0) {
            AcknowledgedResponse delete = orangeRestClient.signatureAuth().selectors().delete(list.get(0).getId());
